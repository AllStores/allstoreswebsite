...
public class ViewHelper {
	...
	public static String getViewName(String baseViewName,
			HttpServletRequest request) {
		Map<String, String> requestHeaders = getRequestHeaders(request);
		String userAgent = requestHeaders.get("user-agent");
		return getViewName(baseViewName, userAgent);
	}

	public static String getViewName(String baseViewName, String userAgent) {
		String finalView = baseViewName;

		boolean isMobileDevice = isMobileDevice(userAgent);
		if (isMobileDevice) {
			finalView = "mobile/" + baseViewName;
		}
		return finalView;
	}
	...
	private static boolean isMobileDevice(String userAgentStr) {
			...
			try {
				...
				if (ua.matches("(?i).*((android|bb\\d+|meego).+mobile|avantgo|bada\\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od|ad)|iris|kindle|lge |maemo|midp|mmp|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\\.(browser|link)|vodafone|wap|windows (ce|phone)|xda|xiino).*")
						|| (ua.length() >= 4 && ua
								.substring(0, 4)
								.matches(
										"(?i)1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\\-)|"
												...
												+ "i230|iac( |\\-|\\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|"
												...
												+ "pan(a|d|t)|pdxg|pg(13|\\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\\-2|po(ck|rt|se)|"
												+ "prox|psio|pt\\-g|qa\\-a|qc(07|12|21|32|60|\\-[2-7]|i\\-)|qtek|r380|r600|raks|"
					...
					logger.info("Mobile browser detected. Overriding redirect mode for agent - "
							+ ua);
					return true;
				}
			} catch (Exception e) {
				logger.error("Exception occured while checking mobile device - "
						+ e.toString());
				return false;
			}
		}
